...
function RegisterForm() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    sessionStorage.setItem("auth_debug_flow", "register");
    const pendingDebug = {
      flow: "register",
      apiMs,
      navStart,
    ...
    console.info("[auth-debug] register pending", pendingDebug);
    setTimingInfo(`注册 API 耗时 ${apiMs}ms，正在跳转...`);
  ...
  const handleGithubSignIn = async () => {
    ...
    const result = await authClient.signIn.social({
      ...
      errorCallbackURL: "/register?oauth=github-error",
    });
    if (result?.error) {
  ...
  return (
    <main
      style={{
        ...
        padding: "40px 20px",
        background:
          "radial-gradient(circle at 80% 0%, #F2E2D0 0%, #F6F1EA 42%, #F6F1EA 100%)",
      }}
    >
      <div style={{ maxWidth: 560, margin: "0 auto" }}>
        ...
        >
          <h1 style={{ margin: 0, fontSize: 32, color: "#1C1915", letterSpacing: "-0.02em" }}>创建账号</h1>
          <p style={{ margin: "10px 0 20px", color: "#7C7264", fontSize: 14 }}>
            注册后你可以更方便地管理和回看自己的 AI 菜谱。
          </p>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            >
              {oauthLoading ? "正在跳转 GitHub..." : "使用 GitHub 注册/登录"}
            </button>
            {timingInfo ? (
          ...
          <p style={{ margin: "16px 0 0", color: "#766A5B", fontSize: 14 }}>
            已有账号？{" "}
            <Link href="/login" style={{ color: "#B8432E", textDecoration: "none", fontWeight: 600 }}>
              去登录
            </Link>
          </p>
        </section>
